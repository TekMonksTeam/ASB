...
function clone(object, skipProperties=[]) {
    if (typeof object !== "object") {ASBLOG.error(`Asked to clone non-object ${object}, returning same back.`); return object;}
    if (!skipProperties.length) try {return JSON.parse(JSON.stringify(object))} catch (err) {
        ASBLOG.error(`Error cloning object, returning shallow clone. The error was ${err}`);
        return {...object};
    }
